$(document).ready(function() {
    ...
    var ur2ed = "AKfycbzQb1AFfuHBzUQZx-OYWzoMa-wGbrgwY13_nsVw9ndaV_57Mr--ondYLkpUJKVjSmn-5w";
    ...
    $.getJSON(url, function(json) {
        ...
        for (z; z < maxsearchNote; z++) {
          topicNote.push(json.records[z].FName + " "+json.records[z].LName+"<img class='serchpropic' src="+json.records[z].ProfilePic+">");
        }
        var topic = topicNote;
...
if(serched == 0 && serchsub == "Subject-" && serchbrd == "Board-" && serchcls == "Class-"){
    ...
    $.getJSON(url, function(json) {
        ...
        for (v; v < maxReVid - 1; v++) {
         ...
         var clsrc = json.records[v].Class ;
          ...
          if (serched === namesrc || serchsub === subsrc || serchbrd ===  brdsrc || serchcls === clsrc) {
            document.getElementById('loader_srced').style.display = "none";
document.getElementById('falseback').style.display ="block";
document.getElementById('prorescon').style.display ="block";
document.getElementById('preedusrc').innerHTML += "<div class='prosrccon'><span class='edusrcprotitle'>"+namesrc+
"</span><img class='serchpropic' src="+json.records[v].ProfilePic+"><button onclick='addeducator(this);' class='addedbtn' class='btn btn-light'>"+
"Add Educator</button><br><br>&#9679; "+ json.records[v].Board + " &#9679; "+json.records[v].Class+
" &#9679; "+json.records[v].Subject+"</div><input class='edaddid' style='display: none;' value='"+
...
if(flag==0){
  ...
  function ctrlqaddst(){
    ...
    var ur2="AKfycbyF2ru_7oBEtZ-XXql6a1OSXA9F1aDT_AEI30gJPR7uL96DKcBPtac_n4kPuEqzSFru";
    var ur3 ="AKfycbwgG73YMyqYuShYfskdaDg196gf7JXwFvgblx6k5HoUPnHQyfeZEZ_CeGvUu1udyiKj";
    var urlstside = ur1+ur3+"/exec" + "?callback=ctrlqadded&eduid=" + addeduid + "&stuid=" + studid + "&action=upedust";
    var request = jQuery.ajax({
      crossDomain: true,
      url: urlstside,
      method: "GET",
